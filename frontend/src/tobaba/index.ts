...
import {
    canvas, ctx,
    gameData, characters,
    COUNTDOWN_INTERVAL, BASE_BALL_SPEED
} from './data';
import { update } from './game';
import { drawRect, drawBall, drawScore, drawMenu, drawGameOver, drawPauseMenu, drawCountdown, drawCharacterSelect } from './draw';
import {
...
function updateTimers(timestamp: number) {
    ...
    if (gameData.gameState === 'game') {
        if (gameData.player1.cooldownTimer < characters[gameData.player1CharIndex].cooldown) {
            gameData.player1.cooldownTimer = Math.min(characters[gameData.player1CharIndex].cooldown, gameData.player1.cooldownTimer + deltaTime);
        }
        if (gameData.player2.cooldownTimer < characters[gameData.player2CharIndex].cooldown) {
            gameData.player2.cooldownTimer = Math.min(characters[gameData.player2CharIndex].cooldown, gameData.player2.cooldownTimer + deltaTime);
        }
...
document.addEventListener('keydown', (e) => {
    ...
    if (gameData.gameState === 'game' || gameData.gameState === 'countingDown') {
        if (e.key.toLowerCase() === 'f') {
            ...
            if (char.abilityName === "Shot" && gameData.player1.cooldownTimer >= char.cooldown) {
        ...
        if (e.key.toLowerCase() === 'j') {
            ...
            if (char.abilityName === "Shot" && gameData.player2.cooldownTimer >= char.cooldown) {
                gameData.ball.speedY = -gameData.ball.speedY;
                gameData.ball.isInverted = !gameData.ball.isInverted;

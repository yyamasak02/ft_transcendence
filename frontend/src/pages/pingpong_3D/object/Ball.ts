...
export class Ball {
  ...
  constructor(scene: Scene, initialPosition: Vector3, baseSpeed: number) {
    ...
    this.ballTrack = new ballTrack(scene, this.mesh);
  }
  ...
  update(
    ...
    const sparkCallback = (position: Vector3) => this.ballTrack.spark(position);
    return updateBallImp(
      this,
      deltaTime,
      paddle1,
      paddle2,
  ...
  reflect(
    ...
    sparkCallback: (position: Vector3) => void,
  ) {
...
function updateBallImp(
  ...
  if (checkPaddleCollision(ball.mesh, paddle1))
    ball.reflect(paddle1, false, sparkCallback);
  // paddle2
  if (checkPaddleCollision(ball.mesh, paddle2))
    ball.reflect(paddle2, true, sparkCallback);
  ...
  if (ball.mesh.position.z > halfHeight - margin) {
    ...
    ball.velocity.z *= -1;
    sparkCallback(ball.mesh.position);
  }
  if (ball.mesh.position.z < -halfHeight + margin) {
    ball.mesh.position.z = -halfHeight + margin;
    ball.velocity.z *= -1;
...
function reflectBallImp(
  ...
  const offsetZ =
    (mesh.position.z - paddle.mesh.position.z) / (paddle.length / 2);
  ...
  sparkCallback(mesh.position);

  // めり込み防止
  if (isLeftPaddle) {
    mesh.position.x =
      paddle.mesh.position.x + (PADDLE_THICKNESS / 2 + BALL_RADIUS + 0.1);
  } else {
    mesh.position.x =
      paddle.mesh.position.x - (PADDLE_THICKNESS / 2 + BALL_RADIUS + 0.1);
  }
...
// プレイ開始時のball positionと射出角度 //////////////////////////
function resetBallImp(
  ball: Ball,
  startFrom: 1 | 2 | "center",
  paddle1: Paddle,
  paddle2: Paddle,
) {
  let x = 0;

...
let activeParticleSystems: ParticleSystem[] = [];
...
function createFireColumn(
  ...
  const particleSystem = new ParticleSystem("winFireColumn", 300, scene);
...
export function createWinEffect(scene: Scene, winner: 1 | 2): void {
  ...
  const winnerColor = new Color4(1, 0.8, 0.2, 0.5);
  ...
  const emitterY = 0.01;
  const zPositions = [COURT_HEIGHT / 2 - 1, -(COURT_HEIGHT / 2 - 1)];
...
export function disposeWinEffect(): void {
